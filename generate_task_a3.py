...
def predict_category(dialog_str):
    global classification_model

    inputs_dict = classification_tokenizer(
        dialog_str, max_length=512, padding="max_length", truncation=True, return_tensors="pt"
    )
    input_ids = inputs_dict.input_ids.to("cuda")
    attention_mask = inputs_dict.attention_mask.to("cuda")

    return CATEGORIES[torch.argmax(classification_model(input_ids, attention_mask, torch.zeros_like(input_ids, device=torch.device('cuda')))).item()]
...
def predict_summary(dialog_str):
    ...
    dialog_len = len(word_tokenize(dialog))
    # See length-summary.ipynb
    max_length = round(0.55 * dialog_len + 18)
...
with open('taskA_gersteinlab_run3.csv', 'w') as file:
